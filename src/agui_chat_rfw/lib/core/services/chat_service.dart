...
class ChatNotifier extends StateNotifier<ChatState> {
  ...
  void addGenUiMessage(GenUiContent content) {
    debugPrint('ChatNotifier: Adding GenUI message - widget: ${content.widgetName}');
    final message = ChatMessage.genUi(
      user: ChatUser.agent,
      content: content,
    );
    debugPrint('ChatNotifier: Created message type: ${message.type}, genUiContent: ${message.genUiContent != null}');
    state = state.copyWith(
      messages: [...state.messages, message],
    );
  ...
  void removeMessage(String messageId) {
    ...
    state = state.copyWith(
      messages: messages,
      isAgentTyping: state.currentStreamingMessageId == messageId ? false : state.isAgentTyping,
      currentStreamingMessageId: state.currentStreamingMessageId == messageId ? null : state.currentStreamingMessageId,
    );
  }
